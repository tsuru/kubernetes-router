...
func main() {
	...
	ingressDomainSuffix := flag.String("ingress-domain-suffix", "local", "Default domain to be used on created vhosts, local is the default. (eg: serviceName.local)")
	...
	for _, mode := range runModes {
		...
		case "istio-gateway":
			localBackend.Routers[mode] = &kubernetes.IstioGateway{
				BaseService:     base,
				DomainSuffix:    *ingressDomainSuffix,
